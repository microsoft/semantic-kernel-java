...
class AzureAISearchVectorStoreCollectionSearchMapping {
    public static String buildFilterString(BasicVectorSearchFilter basicVectorSearchFilter,
        ...
        if (basicVectorSearchFilter == null || basicVectorSearchFilter.getFilterClauses().isEmpty()) {
            return "";
        }

        return String.join(" and ", basicVectorSearchFilter.getFilterClauses().stream().map(filterClause -> {
                    if (filterClause instanceof AzureAISearchEqualityFilterClause) {
                        ...
                        return new AzureAISearchEqualityFilterClause(storageNames.get(azureFilterClause.getFieldName()), azureFilterClause.getValue()).getFilter();
                        ...
                        // Create new instance with the storage name of the field
                        return new AzureAISearchTagListContainsFilterClause(storageNames.get(azureFilterClause.getFieldName()), azureFilterClause.getValue()).getFilter();
                    } else {
