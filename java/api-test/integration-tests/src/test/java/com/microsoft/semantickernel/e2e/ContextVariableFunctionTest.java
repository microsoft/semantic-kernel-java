        ...
        return chat.invokeAsync(context, null)
            .map(
                result -> {
                    ...
                    ContextVariable<String> existingHistoy = finalContext.getVariables()
                        .get("history", String.class);
                    if (existingHistoy == null) {
                        existingHistoy = ContextVariable.of("");
                    }
                    String toAppend = existingHistoy.getValue();
                    toAppend += "\nUser: " + input + "\nChatBot: " + result.getResult() + "\n";
                    return finalContext.setVariable("history", ContextVariable.of(toAppend));
                });
    }
}
